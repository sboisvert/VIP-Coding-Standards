...
namespace StephBoisvertWordPressVIPMinimum\Sniffs\Functions;
	...
	public function findNonCheckedVariables( $stackPtr ) {
		...
		foreach ( $callees as $callee ) {
			...
			if ( true === in_array( $tokens[ $next ]['code'], Tokens::$functionNameTokens, true )
				...
				$phpcsFile->addWarning( sprintf( 'Type of %s must be checked before calling %s using that variable', $variableName, $callee ), $next, 'CheckReturnValue', false, 3 );
				return;
			}
		}
	}
