<?php
/**
 * StephBoisvertWordPressVIPMinimum Coding Standard.
 *
 * @package VIPCS\StephBoisvertWordPressVIPMinimum
 */

namespace StephBoisvertWordPressVIPMinimum\Sniffs\VIP;
...
class RestrictedFunctionsSniff extends \WordPress\Sniffs\VIP\RestrictedFunctionsSniff {
	...
	public function getGroups() {
		...
		$new_groups = array(
			...
			'rewrite_rules'            => array(
				'type'      => 'warning',
				'message'   => '%s should only be used when something is changed and not on a hook that runs on pageload',
				'functions' => array(
					'flush_rewrite_rules',
				),
				false,
			...
			'attachment_url_to_postid' => array(
				'type'      => 'error',
				'message'   => '%s() is does not cache, please use wpcom_vip_attachment_url_to_postid() instead.',
				'functions' => array(
